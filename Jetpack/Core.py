...
display.set_caption('Clicky Car')
clock = time.Clock()
...
images = {
    'start_thing': image.load('images//start.png'),
    'background': image.load('images//blank.png'),
    'ground': image.load('images//blank.png'),
    'pole': image.load('images//box.png'),
    'score_board': image.load('images//score.png')
}
...
class Flappy_Class:
    ...
    def __init__(self):
        ...
        self.flap_1 = transform.scale(image.load('images//guy1.png'), (self.width * 2, self.height * 2))
        self.flap_2 = transform.scale(image.load('images//guy2.png'), (self.width * 2, self.height * 2))
        self.flap_3 = transform.scale(image.load('images//guy3.png'), (self.width * 2, self.height * 2))
    ...
    def show(self, rotate):
        ...
        if rotate:
            angle = min(min(max_angle, self.velocityY * 2), self.old_angle + 20)
...
def show_backdrop(speed):
    ...
    if len(backdrops) == 0:
        num_of_backdrops = math.ceil(windowX / math.ceil(scale * rect.width / 2)) + 1

        for x in range(num_of_backdrops):
            backdrops.append(math.floor(scale * rect.width) * x)

    img = transform.scale(images['background'], (math.ceil(scale * rect.width / 2), math.ceil(scale * rect.height / 2)))
...
my_font_2 = font.SysFont('arial', 50)
my_font_3 = font.SysFont('arial', 30)
